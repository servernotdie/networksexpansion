...
import com.ytdd9527.networks.expansion.core.item.machine.stackmachine.utils.RecipeUtil;
import com.ytdd9527.networks.expansion.core.item.machine.stackmachine.utils.YamlUtil;
...
public class StackMachine extends AbstractStackMachine {
    ...
    public static void loadConfig() throws IOException {
        ...
        for (String fileName : files.keySet()) {
            ...
            YamlConfiguration configuration = YamlUtil.getYamlConfiguration(path, fileName);
            ...
            for (Object recipeContent : recipesContent) {
                ...
                if (inputs != null) {
                    for (Object input : inputs) {
                        ...
                        ItemStack item = YamlUtil.readItem(inputSection);
                ...
                if (outputs != null) {
                    for (Object output : outputs) {
                        ...
                        if (outputSection.getString("type").equals("weight")) {
                            Map<ItemStack, Integer> weightItems = YamlUtil.readItem(outputSection, true);
                            for (ItemStack item : weightItems.keySet()) {
                                outputItems.put(item, item.getAmount());
                                weights.put(item, weightItems.get(item));
                            }
                        }
                        ItemStack item = YamlUtil.readItem(outputSection);
                        outputItems.put(item, item.getAmount());
                    }
                }
