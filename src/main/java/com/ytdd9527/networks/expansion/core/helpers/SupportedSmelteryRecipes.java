package com.ytdd9527.networks.expansion.core.helpers;
...
@UtilityClass
public final class SupportedSmelteryRecipes {
    ...
    public static boolean testRecipe(@Nonnull ItemStack[] input, @Nonnull ItemStack[] recipe) {
        for (int test = 0; test < recipe.length; test++) {
            if (!StackUtils.itemsMatch(input[test], recipe[test], true, true)) {
                return false;
            }
        }
        return true;
